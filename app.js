/**
 * Hệ Thống Điểm Danh Siêu Thị & Boss
 * app.js - Logic điểm danh, copy tag tên @MãNV, thêm/xoá siêu thị
 */
...
(function () {
  ...
  // ==========================================================================
  // 1. DANH SÁCH 24 SIÊU THỊ & BOSS GỐC THEO YÊU CẦU
  // ==========================================================================
  ...
  let state = {
    stores: [],
    attendance: {}, // Cấu trúc: { "YYYY-MM-DD": { [storeId]: true/false } }
  ...
  function loadData() {
    try {
      ...
      console.error('Lỗi nạp LocalStorage:', err);
      state.stores = [...DEFAULT_STORES];
      state.attendance = {};
    }
  }
  ...
  // ==========================================================================
  // 4. TRÍCH XUẤT TAG CÚ PHÁP @MãNV (VÍ DỤ: "Khắc_30653" -> "@30653")
  // ==========================================================================
  function extractTag(bossStr) {
    ...
    // Nếu không có dấu gạch dưới, tìm chuỗi số hoặc lấy toàn bộ
  ...
  // ==========================================================================
  // 6. RENDER BẢNG ĐIỂM DANH (CÁC CỘT: STT, SIÊU THỊ, BOSS, CHECK, TAG TÊN, XOÁ)
  // ==========================================================================
  ...
  // ==========================================================================
  // 8. ĐIỂM DANH: TOGGLE, CHECK TẤT CẢ, BỎ CHECK
  // ==========================================================================
  function toggleCheck(storeId) {
    ...
    state.attendance[state.selectedDate][storeId] = !current;
  ...
  function uncheckAll() {
    if (!state.attendance[state.selectedDate]) return;

    state.stores.forEach(st => {
      state.attendance[state.selectedDate][st.id] = false;
    });
  ...
  // ==========================================================================
  // 9. COPY TAG TÊN VÀO BỘ NHỚ TẠM (CLIPBOARD)
  // ==========================================================================
  ...
  // Copy danh sách tag của tất cả những người CHƯA CHECK
  function copyUncheckedTags() {
    ...
    // Loại bỏ tag trùng nhau nếu 1 boss phụ trách nhiều siêu thị
  ...
  function handleSaveStore(e) {
    ...
    renderTable();
    updateStats();
    showToast(`Đã thêm siêu thị "${name}" thành công!`, 'success');
  }
  ...
  function confirmDeleteStore() {
    ...
    // Xoá record điểm danh liên quan
    Object.keys(state.attendance).forEach(date => {
      if (state.attendance[date] && state.attendance[date][storeId] !== undefined) {
        delete state.attendance[date][storeId];
      }
    });
  ...
  function resetToDefault() {
    if (confirm('Bạn có chắc chắn muốn khôi phục lại danh sách gốc 24 siêu thị ban đầu?')) {
      state.stores = [...DEFAULT_STORES];
      saveStores();
      renderTable();
      updateStats();
  ...
  // ==========================================================================
  // 11. XUẤT CSV EXCEL & SAO LƯU DỮ LIỆU
  // ==========================================================================
  function exportCSV() {
    const dayRecords = state.attendance[state.selectedDate] || {};
    const rows = [
  ...
  // ==========================================================================
  // 12. SỰ KIỆN GIAO DIỆN (EVENT LISTENERS)
  // ==========================================================================
  function setupEventListeners() {
    ...
    // Sự kiện trong bảng (Event Delegation)
    const tbody = document.getElementById('attendance-table-body');
    tbody.addEventListener('click', (e) => {
      // Bấm nút Check / Chưa Check
      const checkBtn = e.target.closest('.btn-check-toggle');
      if (checkBtn) {
        const storeId = checkBtn.dataset.id;
        toggleCheck(storeId);
        return;
      }
      ...
      if (tagBtn) {
        ...
        copyToClipboard(tagText, tagBtn);
        return;
      }
      ...
      if (deleteBtn) {
        ...
        promptDeleteStore(storeId);
      }
    });
  }

  // ==========================================================================
  // 13. TIỆN ÍCH (TOAST & ESCAPE HTML)
  // ==========================================================================
